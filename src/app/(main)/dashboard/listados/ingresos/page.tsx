...
function buildCsv(items: Income[]) {
  const headers = ["Fecha", "Cliente", "Tipo", "Categoria", "Metodo", "Monto", "Referencia", "Descripcion"];
...
function getTypeLabel(value: string) {
  if (value === "PAGO CLIENTE") return "Cobro de cliente";
  if (value === "VENTA PAPELERIA") return "Venta de papelería";
  if (value === "OTRO INGRESO") return "Ingreso contable";
  return value;
}
...
export default function IncomesPage() {
  ...
  const chartGridStroke = isDarkTheme ? "rgba(148,163,184,0.16)" : "#dbe4ea";
  const chartCursorFill = isDarkTheme ? "rgba(16,185,129,0.12)" : "rgba(16, 185, 129, 0.06)";
  const tooltipStyle = {
    borderRadius: 16,
    border: isDarkTheme ? "1px solid rgba(52,211,153,0.16)" : "1px solid rgba(16,185,129,0.12)",
  ...
  const incomeBarFill = isDarkTheme ? "#34d399" : "#6ee7b7";
  const incomeAreaStroke = isDarkTheme ? "#34d399" : "#059669";
  const incomeLineStroke = isDarkTheme ? "#a7f3d0" : "#047857";
  ...
  const fetchIncomes = async (signal?: AbortSignal) => {
    setIsLoading(true);
    ...
    try {
      ...
      const response = await fetch(`/api/ingresos${query ? `?${query}` : ""}`, { signal, cache: "no-store" });
      const data: ApiResponse = await response.json();

      if (!data.success) {
        toast.error(`Error al cargar ingresos: ${data.error ?? "Error desconocido"}`);
        return;
      }
  ...
  useEffect(() => {
    ...
    void fetchIncomes(controller.signal);
  ...
  const paginatedItems = incomes.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
  ...
  const handleExport = () => {
    if (!incomes.length) {
      toast.error("No hay ingresos para exportar");
      return;
    }

    const blob = new Blob([buildCsv(incomes)], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `reporte-ingresos-${summary.range.startDate || "actual"}-${summary.range.endDate || "actual"}.csv`;
    link.click();
  ...
  return (
    <div className="animate-in fade-in flex flex-col gap-6 p-2 duration-500">
      <Card className="overflow-hidden border border-emerald-500/10 bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.22),_transparent_45%),linear-gradient(135deg,_rgba(255,255,255,0.96),_rgba(236,253,245,0.96))] shadow-xl ring-1 ring-emerald-500/10 dark:border-emerald-400/15 dark:bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.2),_transparent_40%),linear-gradient(135deg,_rgba(2,6,23,0.96),_rgba(6,78,59,0.32))] dark:ring-emerald-400/10">
        <CardContent className="flex flex-col gap-6 p-6 md:flex-row md:items-end md:justify-between">
          <div className="space-y-3">
            <Badge className="w-fit border border-emerald-500/15 bg-emerald-500/10 text-emerald-700 hover:bg-emerald-500/10 dark:border-emerald-400/25 dark:bg-emerald-400/10 dark:text-emerald-200">
              <Sparkles className="mr-1 h-3.5 w-3.5" /> Panel de precisión
            </Badge>
            <div>
              <h1 className="text-foreground text-3xl font-bold tracking-tight">Reporte de Ingresos</h1>
              <p className="text-muted-foreground mt-2 max-w-2xl">
                Filtros por fecha, categoría, tipo, método, monto y búsqueda libre con agregados calculados sobre el
            ...
            <div className="flex flex-wrap gap-2 text-xs text-slate-600 dark:text-slate-300">
              <Badge
                variant="outline"
                className="border-emerald-200 bg-white/70 text-emerald-700 dark:border-emerald-400/20 dark:bg-slate-950/40 dark:text-emerald-200"
              >
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              className="gap-2 border-emerald-500/20 bg-white/70 text-emerald-700 hover:bg-emerald-50 dark:border-emerald-400/20 dark:bg-slate-950/40 dark:text-emerald-200 dark:hover:bg-emerald-500/10"
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card className="border-none shadow-md ring-1 ring-emerald-100">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-slate-600">Ingresos filtrados</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-emerald-700">{formatCurrency(summary.total)}</div>
            <p className="mt-1 text-xs text-slate-500">Suma exacta del filtro actual</p>
          </CardContent>
        </Card>
        <Card className="border-none shadow-md ring-1 ring-blue-100">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="mt-1 text-xs text-slate-500">Operaciones dentro del rango y filtros</p>
          </CardContent>
        </Card>
        <Card className="border-none shadow-md ring-1 ring-indigo-100">
          ...
          <CardContent>
            ...
            <p className="mt-1 text-xs text-slate-500">Promedio por ingreso</p>
          </CardContent>
        </Card>
        <Card className="border-none shadow-md ring-1 ring-violet-100">
          <CardHeader className="pb-2">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.9fr)]">
        <Card className="border-none shadow-xl ring-1 ring-emerald-100">
          <CardHeader>
            ...
            <CardDescription>
              Curva principal con columnas de respaldo para detectar picos y cambios reales dentro del rango filtrado.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={dailyStats} margin={{ top: 12, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [formatCurrency(value), "Ingreso del día"]}
                  ...
                  <Area
                    dataKey="total"
                    type="monotone"
                    stroke={incomeAreaStroke}
                    fill="url(#income-fill)"
                    strokeWidth={3}
                  />
                  <Line dataKey="total" type="monotone" stroke={incomeLineStroke} strokeWidth={1.5} dot={false} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="bg-card/95 border border-slate-200/70 shadow-xl ring-1 ring-slate-200/70 dark:border-slate-800 dark:bg-slate-950/70 dark:ring-slate-800/80">
          <CardHeader>
            ...
            <CardDescription>Las categorías y métodos también respetan fechas, montos y búsqueda.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="outline"
                  className="border-emerald-200 text-emerald-700 dark:border-emerald-400/20 dark:text-emerald-200"
                >
      ...
      <Card className="bg-card/95 border border-slate-200/70 shadow-md ring-1 ring-slate-200/70 dark:border-slate-800 dark:bg-slate-950/70 dark:ring-slate-800/80">
        <CardHeader className="border-b bg-slate-50/70 pb-4 dark:border-slate-800 dark:bg-slate-900/70">
          ...
          <CardDescription>
            Todo lo que ves en la tabla, totales y gráfica sale del mismo conjunto filtrado.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-5">
          <div className="relative xl:col-span-2">
            ...
            <Input
              ...
              className="pl-9"
              placeholder="Buscar cliente, referencia, categoría, método..."
            />
          </div>
      ...
      <Card className="bg-card/95 overflow-hidden border border-slate-200/70 shadow-md ring-1 ring-slate-200/70 dark:border-slate-800 dark:bg-slate-950/70 dark:ring-slate-800/80">
        <CardHeader className="flex flex-col gap-4 border-b bg-slate-50/70 pb-5 md:flex-row md:items-center md:justify-between dark:border-slate-800 dark:bg-slate-900/70">
          ...
          <Badge
            variant="outline"
            className="w-fit border-emerald-200 bg-white text-emerald-700 dark:border-emerald-400/20 dark:bg-slate-950/40 dark:text-emerald-200"
          >
              ...
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={6} className="py-20 text-center text-slate-500 italic dark:text-slate-300">
                    Recalculando ingresos con los filtros aplicados...
                  </TableCell>
                </TableRow>
              ) : !paginatedItems.length ? (
                <TableRow>
                  <TableCell colSpan={6} className="py-20 text-center text-slate-500 dark:text-slate-300">
                    No se encontraron ingresos con los criterios actuales.
                  </TableCell>
                </TableRow>
              ) : (
                paginatedItems.map((income) => (
                  <TableRow key={income.id} className="hover:bg-emerald-50/40 dark:hover:bg-emerald-500/5">
                    <TableCell className="px-4 py-3">
                      <div className="flex flex-col">
                        ...
                        <span className="flex items-center gap-1 text-[11px] tracking-wide text-slate-500 uppercase dark:text-slate-400">
                          ...
                          {formatDate(income.fecha, { weekday: "short" })}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div
                          className={`rounded-xl p-2 ${income.tipo === "PAGO CLIENTE" ? "bg-blue-500/10 text-blue-600" : income.tipo === "VENTA PAPELERIA" ? "bg-amber-500/10 text-amber-700" : "bg-violet-500/10 text-violet-700"}`}
                        >
                          {income.tipo === "PAGO CLIENTE" ? (
                        ...
                        <div>
                          <p className="font-semibold text-slate-900 dark:text-slate-50">{income.cliente}</p>
                          <p className="text-xs tracking-wide text-slate-500 uppercase dark:text-slate-400">
                            {getTypeLabel(income.tipo)}
                          </p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="px-4 py-3">
                      <Badge
                        variant="outline"
                        className="border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-500/10 dark:text-emerald-200"
                      >
                        {income.categoria}
                      </Badge>
                    </TableCell>
                    <TableCell className="px-4 py-3">
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-slate-500 dark:text-slate-400">
                          {income.referencia ? `Ref. ${income.referencia}` : `Cobrador: ${income.cobrador}`}
                        </p>
                      </div>
                    </TableCell>
                    <TableCell className="px-4 py-3">
