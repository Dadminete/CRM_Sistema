                  ...
                  clients.map((client) => (
                    <TableRow key={client.clienteId}>
                      ...
                      {MONTHS.map((month) => {
                        ...
                        const isPaid = status.estado === "pagado";
      ...
      {!isLoading && clients.length > 0 ? (
        <Card>
          ...
          <CardContent className="text-sm">
            Total clientes listados: <strong>{clients.length}</strong>
            <br />
            Año evaluado: <strong>{year}</strong>
            <br />
            Mes filtrado: <strong>{month === "all" ? "Todos" : MONTHS.find((item) => String(item.value) === month)?.short ?? month}</strong>
          </CardContent>
        </Card>
      ) : null}
    </div>
