  ...
  try {
    ...
    const getCajaPrincipalData = async () => {
      ...
      return {
        ...
        ultimosDias: historyRaw.rows.map((r) => ({
    ...
    const getPapeleriaData = async () => {
      ...
      const [ultimaSesion, ventasMes, gastosMes, historyRaw] = await Promise.all([
        db
          .select({ estado: sesionesCaja.estado })
          .from(sesionesCaja)
          .where(eq(sesionesCaja.cajaId, boxId))
          .orderBy(desc(sesionesCaja.fechaApertura))
          .limit(1),
        db
          .select({ total: sql<number>`COALESCE(SUM(CAST(${movimientosContables.monto} AS DECIMAL)), 0)` })
    ...
    const getBankData = async () => {
      ...
      const stats = (accountStats.rows[0] as Record<string, unknown> | undefined) || {
        total_inicial: 0,
        total_ingresos: 0,
        total_gastos: 0,
      };
      ...
      const rows = monthlyStatsRaw.rows.slice().reverse();
      const ultimosMeses = rows
        .map((r) => {
          const d = new Date(String(r.mes_fecha));
          const res = {
    ...
    const getInvoiceData = async () => {
      const [balances, dailyIncome] = await Promise.all([
        db
          .select({ facturaEstado: facturasClientes.estado, montoPendiente: cuentasPorCobrar.montoPendiente })
      ...
      balances.forEach((b) => {
        ...
        if (estado === "pendiente") {
          ...
          totalParcial += m;
        } else if (estado === "adelantado" || estado === "pago adelantado") {
          totalAdelantado += m;
        }
      });
    ...
    const getNetIncomeData = async () => {
      const res = await db
        .select({ total: sql<number>`COALESCE(SUM(CAST(${suscripciones.precioMensual} AS DECIMAL)), 0)` })
        .from(suscripciones)
